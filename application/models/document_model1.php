...
{    
	...
	function statementItems($customer_id,$date=false) 
	{
		...
		if($date)
		{
			$whare=' and invoices.date_issue <="'.date("Y-m-d",strtotime($date)).'"';
		}
	$sql='SELECT *,IF(type="partial","Amount Brought Fwd From '.date("Y-m-d",strtotime($date.'-1 month')).'",CONCAT("INV",invoice_id)) as description, (SELECT SUM(amount) FROM customer_payments WHERE customer_id=cp.customer_id AND status="pending" AND payment_id<=cp.payment_id) as balance ,(select sum(price) from i_items where invoice_id=invoices.invoice_id and returned="Y") as credited
			  FROM customer_payments cp JOIN invoices USING(invoice_id)
			  WHERE cp.status="pending" AND invoices.active="Y" AND invoices.user_id IN(SELECT user_id FROM users WHERE outlet_id="'.$this->session->userdata('outlet_id').'") and invoices.customer_id="'.$customer_id.'" '. $whare.'
