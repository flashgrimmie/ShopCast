...
{    
	...
	function statementItems($customer_id) 
	{
		$sql='SELECT *,IF(type="partial","BALANCE",CONCAT("INV",invoice_id)) as description, (SELECT SUM(amount) FROM customer_payments WHERE customer_id=cp.customer_id AND status="pending" AND payment_id<=cp.payment_id) as balance 
			  FROM customer_payments cp JOIN invoices USING(invoice_id)
			  WHERE cp.status="pending" AND invoices.active="Y" AND invoices.user_id IN(SELECT user_id FROM users WHERE outlet_id="'.$this->session->userdata('outlet_id').'")
