  ...
  */
  class Datatables
  {
    ...
    */
    ...
    {
		//changes for remove field from search
      $remove_search_fields_arr=array('qty','cost_price','sell_price','last_sell_price','stock.item_id','location','unit','supplier_id');
      ...
      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++)
		  ...
		  {
			$pos=strpos($this->select[$mColArray[$i]],"GROUP_CONCAT");
            if(preg_match('/\(/',$this->select[$mColArray[$i]]) && $pos===false) {
              continue;
            }
            if(preg_match('/date/',$this->select[$mColArray[$i]])){
                $sWhere .= $this->select[$mColArray[$i]] . " LIKE '%" . $sSearch . "%' OR DATE_FORMAT(" . $this->select[$mColArray[$i]] . ", '%D %b %Y') LIKE '%" . $sSearch . "%' OR ";
            } else {
	  ...
	  //changes for "Keyword search"  with space
      if($sSearch != ''){
          $extra_sWhere='';
          //break word with space
          $words_arr=explode(' ',$sSearch);
          if(!empty($words_arr) && count($words_arr)>1){
                 $extra_sWhere.='(';
                 foreach($words_arr as $kw=>$kwy){
                      $extra_sWhere.='(';
                      foreach($this->select as $col_name=>$single_col){
                          if(!in_array($col_name,$remove_search_fields_arr)){
                            $extra_sWhere .= $col_name. " LIKE '%".$kwy."%' OR ";
                          }
                      }
                      $extra_sWhere=substr_replace($extra_sWhere, '', -3);
                      $extra_sWhere.=') AND ';
                 }
                 $extra_sWhere=substr_replace($extra_sWhere, '', -4);
                 $extra_sWhere.=')';
                 $sWhere.='OR '.$extra_sWhere;
          }
  ...
  public function customQuery($aColumns,$query,$aSearchColumns=false,$order_by='ORDER BY '){

    if(!$aSearchColumns) {
      $aSearchColumns=$aColumns;
    }

    $iDisplayStart=$this->ci->input->get('iDisplayStart');
    $iDisplayLength=$this->ci->input->get('iDisplayLength');
    $iSortCol_0=$this->ci->input->get('iSortCol_0');
    $iSortingCols=$this->ci->input->get('iSortingCols');
    ...
    if ($iDisplayStart && $iDisplayLength!= '-1') {
      ...
      $sLimit = "LIMIT 0, ".intval($iDisplayLength);
    }
    ...
    for ($i=0; $i<intval($iSortingCols); $i++) {
      if ($this->ci->input->get('bSortable_'.intval($this->ci->input->get('iSortCol_'.$i))) == "true" ) {
        $sOrder .= "".$aSearchColumns[intval($this->ci->input->get('iSortCol_'.$i))]." ".($this->ci->input->get('sSortDir_'.$i)==='asc' ? 'asc' : 'desc') .", ";
      }
    }
    ...
    if ($sSearch!="")
    {
      ...
      for ($i=0 ; $i<count($aSearchColumns) ; $i++) {
        if(preg_match('/date/',$aSearchColumns[$i])){
          $sWhere .=$aSearchColumns[$i]. " LIKE '%" . $sSearch . "%' OR DATE_FORMAT(" .$aSearchColumns[$i]. ", '%D %b %Y') LIKE '%" . $sSearch . "%' OR ";
        } else {
          $sWhere .=" ".$aSearchColumns[$i]." LIKE '%".mysql_real_escape_string($sSearch)."%' OR ";
        }
      }
    ...
    $sQuery = "SELECT * FROM ($query) as der $sWhere $sOrder $sLimit "; 
    $rResult = $this->ci->shared_model->getQuery($sQuery, true);

    $filteredTotal="SELECT count(*) as c FROM ($query) as der $sWhere";
    $iFilteredTotal = $this->ci->shared_model->getRow($filteredTotal);
    $iFilteredTotal = $iFilteredTotal->c;

    $cTotal='SELECT count(*) as c FROM ('.$query.') as der';
    $iTotal = $this->ci->shared_model->getRow($cTotal);
    $iTotal = $iTotal->c;
