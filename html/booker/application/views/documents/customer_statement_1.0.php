    ...
    <div class="col-md-12">
        ...
        <table class="top50">
            <tr>
                ...
                <td>
                    <div style="line-height:24px; font-family:'Open Sans',sans-serif;">A/C
                        CODE:&nbsp;<b><?php echo $customer_details->customer_id; ?></b></div>
                    <div style="line-height:24px; font-family:'Open Sans',sans-serif;">PAGE:&nbsp;<b>1</b></div>
                    <div style="line-height:24px; font-family:'Open Sans',sans-serif;">PRINT
                        DATE:&nbsp;<b><?php echo date('d/m/Y') ?></b></div>
    ...
    <div class="col-md-12">
        <table class="table top10 text-left" style="text-align:left;">
            ...
            <?php
                ...
                $dateCompare = new DateTime($selected_date);
                $dateCompare->modify('first day of this month');
                $dateCompare->setTime(0, 0, 0);

                foreach ($items as $item) {
                    if ($item->opening_balance > 0 && (strtotime($item->date_issue) <= $dateCompare->getTimestamp()) && (date('Y', strtotime($item->date_issue)) == date('Y', strtotime($selected_date)))) {
                    ...
                    if (strtotime($item->date_issue) <= $dateCompare->getTimestamp()) {
                        $c_total = ($item->credited == 'NULL') ? 0 : $item->credited;
                        $credit_blnc = $credit_blnc + $c_total;
                        $debit_blnc = $opening_balance + $debit_blnc +  $item->total;
                ...
                foreach ($items as $item) {
                    ...
                    if (in_array(date('m-Y', strtotime($item->date_issue)), $keys)) {
                        ...
                        $monthly_arr[date('m-Y', strtotime($item->date_issue))] = $monthly_arr[date('m-Y', strtotime($item->date_issue))] + $item->total;
                    } else {
                        $monthly_arr[date('m-Y', strtotime($item->date_issue))] = $item->total;
                    }
                }
                ...
                foreach ($items as $item):
                    ...
                    if ((date('Y-m', strtotime($item->date_issue)) == date('Y-m', strtotime($selected_date)))) {
                        $recursive += $item->total - ($item->credited == '' ? 0 : $item->credited) ;
                    ...
                    if (date('Y-m', strtotime($item->date_issue)) == date('Y-m', strtotime($selected_date))) {
                        ...
                        <td></td>
                        <td><?php echo format_price($item->total); ?></td>
                    ...
                    if ((date('Y', strtotime($item->date_issue)) == date('Y', strtotime($selected_date)))) {
                        $c_total = ($item->credited == 'NULL') ? 0 : $item->credited;
                        $credit_blnc = $credit_blnc + $c_total;
                        $debit_blnc = $debit_blnc + $item->total;
                    }
                    ?>
                    </tr>
                <?php endforeach; ?>
...
<div class="row">
    <div class="col-md-12">
        <?php
            // Current month key
            $temp = explode('-', $selected_date);
            $selected_date_key = $temp[1] . '-' . $temp[0];
            ...
            $previous_month_key = $month.'-'.$year;

            if( !array_key_exists( $selected_date_key, $monthly_arr ) ){
                $monthly_arr[$selected_date_key] = 0;
            }

            if (!array_key_exists($previous_month_key, $monthly_arr)) {
                $monthly_arr[$previous_month_key] = 0;
            }
                        ...
                        if ((date('Y', strtotime('01-' . $key)) == date('Y', strtotime($selected_date)))) {
                            ...
                            <td>
                                <table border="1" style="width:100%;">
                                    <tr>
                                        <th><b><?php echo date('M-Y', strtotime('01-' . $key)); ?></b></th>
                                    </tr>
                                    <tr>
                                        <td><?php echo format_price($month); ?></td>
                                    </tr>
                                </table>
                            </td>
                            <?php        if ($i % 6 == 0) {
...
<div class="row">
    ...
    ?>
    <div class="col-md-12" style="text-align:right;width:100%;"><b>Total
            Balance:</b> <?php echo format_price(($recursive )); ?></div>
</div>
<?php } else { ?>
<div class="col-md-12" style="text-align:right;width:100%;"><b>Total
        Balance:</b> <?php echo format_price($recursive ); ?></div>
        ...
        <table>
            <tr>
                <td class="text-left" width="75%">
                    ...
                    <div style="text-align:center;">MGK Autoparts services</div>
                    <br/>
                </td>
...
<?php
    //echo '<pre>' . print_r( $opening_inv, true) . '</pre>';
?>
</body>
</html>