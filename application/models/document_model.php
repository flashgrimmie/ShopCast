<?php
    ...
    class Document_model extends CI_Model {
        ...
        function invoiceDetails($invoice_id) {
            $sql = 'SELECT i.*,der.*,c.name as customer_name, c.address as customer_address,c.address_1 as customer_address1,
                           c.phone as customer_phone,c.fax as customer_fax, c.email as customer_email, c.attention as attention
				FROM invoices i 
        ...
        function getCredits($cn_id) {
            $sql = 'SELECT * FROM returned_cn WHERE cn_id=' . $cn_id;
            return $this->shared_model->getRow($sql);
        }
	
	function getReturnItem($cn_id) {
			$sql = 'SELECT * FROM returned_items WHERE cn_id=' . $cn_id;
			return $this->shared_model->getRow($sql);
	}

        function getItem( $item_id) {
            $sql = 'SELECT stock_num,description FROM stock WHERE item_id=' . $item_id;
            return $this->shared_model->getRow($sql);
        }

        function getCustomerDetails($id) {
            $sql = 'SELECT * FROM customers WHERE customer_id="' . $id . '"';
            return $this->shared_model->getRow($sql, true);
        }
        ...
        function statementItems($customer_id, $date = false) {
            $where = '';
            if ($date) {
                $where = ' AND date_issue <="' . date("Y-m-d", strtotime($date)) . '"';
            }
            /*$sql =  'SELECT *,CONCAT("INV",invoices.invoice_id) as description,
	                        (SELECT amount FROM customer_payments WHERE invoice_id=invoices.invoice_id AND (status="paid" OR type="partial")) as credited
                      FROM invoices
                      WHERE  invoices.active="Y"
                      ...
                      ORDER BY invoices.date_issue ASC';*/
            $sql = 'SELECT * FROM invoices WHERE  active="Y" AND draft="N"
                            AND user_id IN(SELECT user_id FROM users WHERE outlet_id="' . $this->session->userdata('outlet_id') . '")
                            AND customer_id="' . $customer_id . '" ' . $where . '  ORDER BY date_issue ASC';
        ...
        function customerPayments($customer_id, $date = false) {

            $where = ' AND date <="' . $date . '"';

            $sql = 'SELECT * FROM customer_payment_history
        ...
        /*    $sql = 'SELECT i.*,der.*,c.name as customer_name, c.address as customer_address, c.phone as customer_phone, c.email as customer_email
				...
				JOIN (SELECT u.user_id,host.name as host_outlet, host.address1 as host_address1, host.address2 as host_address2, host.contact as host_contact FROM users u JOIN outlets host USING(outlet_id)) as der ON der.user_id=i.user_id
				WHERE cs_id="' . $cs_id . '"';*/
            /*$sql = 'SELECT * FROM cash_sales WHERE cs_id='.$cs_id;*/
            $sql = 'SELECT i.*,der.*
                FROM cash_sales i 
                JOIN (SELECT u.user_id,u.name as staff,host.name as host_outlet, host.address1 as host_address1, host.address2 as host_address2, host.contact as host_contact FROM users u JOIN outlets host USING(outlet_id)) as der ON der.user_id=i.user_id
                WHERE cs_id="' . $cs_id . '"';
            $query = $this->db->query($sql);
            $result = $query->row_array();
            return $result;
		...
		function rpiItems($rpi_id)
		{
			$sql = 'SELECT * FROM rpi_items JOIN stock USING(item_id) WHERE rpi_id='.$rpi_id; 
			$result = $this->shared_model->getQuery($sql);
			return $result;
		}
		...
		{
			$sql = 'SELECT rpi.*,der.*,s.name as supplier_name, s.address as supplier_address, s.phone as supplier_phone, s.email as supplier_email, s.contact_person as supplier_cp  
					FROM purchase_returns rpi 
					JOIN suppliers s USING(supplier_id) 					
					JOIN (SELECT u.name as staff,u.user_id,host.name as host_outlet, host.location as host_location,host.address1 as host_address1,host.address2 as host_address2, host.contact as host_contact FROM users u JOIN outlets host USING(outlet_id)) as der ON der.user_id=rpi.user_id
					WHERE rpi_id="'.$rpi_id.'"'; 
			$query = $this->db->query($sql);
			$result=$query->row_array();
			return $result;
