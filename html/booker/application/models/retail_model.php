<?php
    ...
    class Retail_Model extends CI_Model {
        ...
        function getInvoices($active = 'Y', $date = false) {
            $outlet_id = $this->session->userdata('outlet_id');
            ...
            $this->datatables->where('invoices.draft', 'N');
            $this->datatables->where('users.outlet_id', $outlet_id);

            if ($date) {
                $issue_date = date('Y-m-d', strtotime($date));
                $d = date_parse_from_format("Y-m-d", $issue_date);
                $days = cal_days_in_month(CAL_GREGORIAN, $d['month'], $d['year']);

                $this->datatables->where('invoices.date_issue between "' . $issue_date . '" and "' . date('Y-m-d', strtotime($date . '+' . ($days - 1) . ' days')) . '"');
            }
		...
		function getVoidInvoices($active = 'Y', $date = false) {
            $outlet_id = $this->session->userdata('outlet_id');

            $this->load->library('datatables');
            $this->datatables->select('invoice_id, date_issue, customers.name,customers.car_plate, total, deposit, opening_balance,partial, status')
                ->from('invoices');
            $this->datatables->join('customers', 'invoices.customer_id=customers.customer_id', 'left');
            $this->datatables->join('users', 'invoices.user_id=users.user_id');
            $this->datatables->where('invoices.active', $active);
            $this->datatables->where('invoices.draft', 'Y');
            $this->datatables->where('users.outlet_id', $outlet_id);
		...
		function getVoidDOs($active = 'Y', $date = false) {
            $outlet_id = $this->session->userdata('outlet_id');

            $this->load->library('datatables');
            $this->datatables->select('do_id, date, customers.name, total, deposit, status')
                ->from('delivery_orders');
            $this->datatables->join('customers', 'delivery_orders.customer_id=customers.customer_id', 'left');
            $this->datatables->join('users', 'delivery_orders.user_id=users.user_id');
            $this->datatables->where('delivery_orders.active', $active);
            //$this->datatables->where('delivery_orders.published', '0');
            $this->datatables->where('users.outlet_id', $outlet_id);
            ...
            if ($date) {
                ...
                $this->datatables->where('delivery_orders.date between "' . $issue_date . '" and "' . date('Y-m-d', strtotime($date . '+' . ($days - 1) . ' days')) . '"');
            }
		...
		function getVoidCS($active = 'Y', $date = false) {
            $outlet_id = $this->session->userdata('outlet_id');

            $this->load->library('datatables');
            $this->datatables->select('cs_id, date, customers.name, total, discount, remark')
                ->from('cash_sales');
            $this->datatables->join('customers', 'cash_sales.customer_id=customers.customer_id', 'left');
            $this->datatables->join('users', 'cash_sales.user_id=users.user_id');
            $this->datatables->where('cash_sales.active', $active);
            $this->datatables->where('cash_sales.draft', 'Y');
            $this->datatables->where('users.outlet_id', $outlet_id);
            ...
            if ($date) {
                ...
                $this->datatables->where('cash_sales.date between "' . $issue_date . '" and "' . date('Y-m-d', strtotime($date . '+' . ($days - 1) . ' days')) . '"');
            }
        ...
        function getexpVoidTransactions() {
            $outlet_id = $this->session->userdata('outlet_id');
            $this->load->library('datatables');
            $query = 'SELECT CONCAT("I",invoice_id) as reference_id,date_issue as date,IF(customer_id is not null,(SELECT name FROM customers WHERE customer_id=invoices.customer_id),"Undefined") as customer,total,"Invoice" as type,(SELECT name FROM users WHERE user_id=invoices.user_id) as staff,discard_reason
              FROM invoices JOIN users USING(user_id) WHERE draft="Y" AND invoices.active="N" AND users.outlet_id="' . $this->session->userdata('outlet_id') . '"
              UNION ALL
              SELECT CONCAT("RS",cs_id) as reference_id,date,IF(customer_id is not null,(SELECT name FROM customers WHERE customer_id=cash_sales.customer_id),"Undefined") as customer,total,"Retail Sale" as type,(SELECT name FROM users WHERE user_id=cash_sales.user_id) as staff,discard_reason
              FROM cash_sales JOIN users USING(user_id) WHERE draft="Y" and cash_sales.active="N" AND users.outlet_id="' . $this->session->userdata('outlet_id') . '"
              UNION ALL
              SELECT CONCAT("SO",so_id) as reference_id,date,IF(customer_id is not null,(SELECT name FROM customers WHERE customer_id=sales_orders.customer_id),"Undefined") as customer,total,"Sales Order" as type,(SELECT name FROM users WHERE user_id=sales_orders.user_id) as staff,discard_reason
              FROM sales_orders JOIN users USING(user_id) WHERE draft="Y" and sales_orders.active="N" AND users.outlet_id="' . $this->session->userdata('outlet_id') . '"';
            $fields = array('reference_id', 'datef', 'customer', 'total', 'type', 'staff', 'discard_reason');
            $search_fields = array('reference_id', 'date', 'customer', 'total', 'type', 'staff', 'discard_reason');
        ...
        function getCSs($active = 'Y') {
            $outlet_id = $this->session->userdata('outlet_id');
            $this->load->library('datatables');
            $this->datatables->select('cs_id, date,total, cash_sales.remark, draft')
                ->from('cash_sales');
       ...
       /* function getCreditNotes() {
            ...
            $query = 'SELECT *
              FROM 
              (SELECT ii.item_id,users.outlet_id, stock_num,barcode,brand,stock.supplier_id,category,description, date_format(invoice_time_stamp,"%D %b %Y") as date_f, invoice_time_stamp as date, ii.invoice_id as reference, customers.name as customer_vendor,"invoice" as transaction_type, SUM(quantity) as issued, 0 as recived
                ...
                WHERE i.active="Y"  AND users.outlet_id="' . $this->session->userdata('outlet_id') . '" and ii.returned="Y"
                GROUP BY item_id,i.invoice_id
               
			    UNION ALL
                SELECT ii.item_id,users.outlet_id, stock_num,barcode,brand,stock.supplier_id,category,description, date_format(cs_time_stamp,"%D %b %Y") as date_f, cs_time_stamp as date, ii.cs_id as reference, customers.name as customer_vendor,"retail sale" as transaction_type, SUM(quantity) as issued,  0 as recived
                ...
                WHERE i.active="Y" AND ii.returned="Y" AND users.outlet_id="' . $this->session->userdata('outlet_id') . '"
                GROUP BY item_id,i.cs_id
				
				UNION ALL
				select ii.item_id,users.outlet_id,stock.stock_num,barcode,brand,stock.supplier_id,category,description,date_format(ii.date_returned,"%D %b %Y") as date_f, ii.date_returned as date, ii.cn_id as reference, customers.name as customer_vendor,"credit note" as transaction_type, SUM(ii.qty) as issued,  0 as recived
				from returned_cn ii JOIN stock USING(item_id) JOIN customers USING(customer_id) JOIN users USING(outlet_id)
				where users.outlet_id="' . $this->session->userdata('outlet_id') . '" group by ii.item_id
              ) as i
              WHERE 1 ';


            $fields = array('item_id', 'stock_num', 'description', 'date', 'reference', 'customer_vendor', 'transaction_type', 'issued', 'recived');
        ...
        function getCreditNotes(){
            ...
            $this->load->library('datatables');
            $this->datatables->select('cn_id, cn_date, customers.name, customers.address, remarks')
                ->from('returned_cn');
            $this->datatables->join('customers', 'returned_cn.customer_id=customers.customer_id');
            $this->datatables->add_column('5', '', '');
            return $this->datatables->generate();
        }
	
	function getReturnsNotes(){
			$outlet_id = $this->session->userdata('outlet_id');
//OUTLET_PREFIX
			$this->load->library('datatables');
			$this->datatables->select('CONCAT(\''.CSR.'\',cn_id) as cn_id, cn_date, customers.name, customers.address, remarks, cn_id as credit_id', false)
				->from('returned_items');
			$this->datatables->join('customers', 'returned_items.customer_id=customers.customer_id');
			$this->datatables->add_column('5', '', '');
			return $this->datatables->generate();
		}
